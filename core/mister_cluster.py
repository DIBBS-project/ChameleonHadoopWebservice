...
def update_hosts_file(instances, username, key_filename, tmp_folder=None):
    ...
    for instance in instances:
        ...
        generate_template_file("common/update_hosts_file.jinja2", update_hosts_file_path,
                               {"node_name": instance.name, "hosts": remote_hosts})
...
class MisterCluster:
    ...
    def add_node_to_cluster(self, host, master=None):
        ...
        # Generate ssh key
        if not cluster_db_object.private_key and not cluster_db_object.public_key:
        ...
        ssh.exec_command("touch success")
        ...
        time.sleep(5)
        ssh.exec_command("bash prepare_node.sh")
        ...
        time.sleep(5)
        ssh.exec_command("bash configure_node.sh")
