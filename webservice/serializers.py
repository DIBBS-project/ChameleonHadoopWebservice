...
from webservice.models import Site, User, Cluster, Host, Software, Script, Event
...
class SoftwareSerializer(serializers.Serializer):
    ...
    cluster_ids = serializers.SerializerMethodField('software_clusters')
    script_ids = serializers.SerializerMethodField('software_scripts')

    def software_clusters(self, software):
        return map(lambda x: x.id, Cluster.objects.filter(software=software.id))
