...
curl --header "username: $USER" -X GET $REMOTE_HADOOP_WEBSERVICE_HOST/fs/rm/output.txt/

# Clean HDFS jar files and folders used by this example
curl --header "username: $USER" -X GET $REMOTE_HADOOP_WEBSERVICE_HOST/hdfs/rm/test.jar/
curl --header "username: $USER" -X GET $REMOTE_HADOOP_WEBSERVICE_HOST/hdfs/rmdir/user/root/
...
curl --header "username: $USER" -i -X GET $REMOTE_HADOOP_WEBSERVICE_HOST/hdfs/mkdir/user/root/
curl --header "username: $USER" -i -X POST -F 'data=@test.txt' $REMOTE_HADOOP_WEBSERVICE_HOST/hdfs/upload/user/root/input/
...
curl --header "username: $USER" -X GET $REMOTE_HADOOP_WEBSERVICE_HOST/hdfs/mergedir/user/root/output/_/output.txt/
